  ...
  $.ajax({
    ...
    success:function(json){
      ...
      var xlabels = [], type_count = [],colors = [],percent = [];
          for(var i = 0; i < json.length; i++) {
              xlabels.push(json[i]["member_id"]);
              type_count.push(json[i]["thisWeek"]);
    ...
    datasets: [
        {
            ...
            fillColor: "rgba(106,90,205,0.5)",
            strokeColor: "rgba(106,90,205,0.8)",
            highlightFill: "rgba(106,90,205,0.75)",
            highlightStroke: "rgba(106,90,205,1)",
            data: type_count
        }
        ...
        var options =
    {
      ...
      var projectChart = new Chart(rCM).Bar(data,options);
