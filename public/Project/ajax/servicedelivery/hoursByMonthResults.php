...
if (strpos($path,'managedservices') !== false) {
  ...
  FROM         dbo.Time_Entry LEFT OUTER JOIN
                        ...
                        left outer join sr_service on time_entry.sr_service_recid = sr_service.sr_service_recid
                        left outer join sr_board on sr_service.sr_board_recid = sr_board.sr_board_recid
  WHERE member.member_id like "%IT Support%" and (dbo.SR_Board.Board_Name = "BackOffice" or dbo.SR_Board.Board_Name = "Managed Services Requests" or dbo.SR_Board.Board_Name = "" or dbo.SR_Board.Board_Name="LogicMonitor") and  (convert(char(6), dbo.time_entry.date_start, 112) <> convert(char(6), getdate(), 112) and year(time_entry.date_start) > year(getdate())-2)
