...
WHERE     dbo.SO_Type.Description = "Project" AND (dbo.SO_Team.Team_Flag = 0) AND (dbo.SO_Team.Owner_Flag = 1) and dbo.SO_Opp_Status.Description = "Won" and datediff(m,dbo.SO_Opportunity.Date_Close_Expected,getdate())=0  and (dbo.so_forecast_dtl.Opportunity_RecID = dbo.SO_Opportunity.Opportunity_RecID) AND ((dbo.so_forecast_dtl.SO_Forecast_Type_ID = "P") or (dbo.so_forecast_dtl.SO_Forecast_Type_ID = "S")) AND (dbo.so_forecast_dtl.Include_Flag = 1)
