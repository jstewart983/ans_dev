...
//echo $path;
  ...
  if(isset($_GET['range1']) && isset($_GET['range2']) && isset($_GET['company']) && isset($_GET['member'])){
    ...
    and (time_entry.date_entered_utc >="'.$range1.'" and time_entry.date_entered_utc <= "'.$range2.'")';

    $otherQuery = 'SELECT sum(time_entry.hours_actual) as elseHours
    FROM time_entry
    left outer join member on time_entry.member_recid = member.member_recid
    ...
    WHERE member.member_id = "'.$type.'" and (time_entry.date_entered_utc >="'.$range1.'" and time_entry.date_entered_utc <= "'.$range2.'") and (member.member_id = "wblakeburn" or member.member_id = "plane" or member.member_id = "jmorgan" or member.member_id = "bfizer" or member.member_id = "rmillen")';
  ...
  and (sr_service.summary like "%AV%" or sr_service.summary like "%ESET%") and
   ...
   DATEDIFF( ww, dbo.Time_Entry.Date_Start, GETDATE() ) = 0 and (member.member_id = "wblakeburn" or member.member_id = "plane" or member.member_id = "jmorgan" or member.member_id = "bfizer" or member.member_id = "rmillen")
