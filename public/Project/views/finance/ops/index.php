...
heap.load("3593988708");
    ...
    <div style="margin-top:52px;"id="view"class="container">
      <div style="margin-top:80px;" id="dashboard" class="row">
        <div class="col-md-2">
          ...
          <div class="panel panel-default">
            <div class="panel-body">
              ...
              <h1 style="text-align:center;" id="report_count">0 <span class="fa fa-bar-chart"></span></h1>
              <button style="width:100%;text-align:center;" type="button" class="btn btn-success" name="new_report_plus" id="new_report_plus"><span class="fa fa-plus"></span></button>
            </div>
          </div>
        ...
        <div class="col-md-2">
          ...
          <div class="panel panel-default">
            <div class="panel-body">
              ...
              <h1 style="text-align:center;" id="check_count">0</h1>
              <button style="width:100%;text-align:center;" type="button" class="btn btn-success" name="new_check_plus" id="new_check_plus"><span class="fa fa-plus"></span></button>
            </div>
          </div>

        </div>
      </div>
      <div id="add_report" style="margin-top:80px;"class=" hidden row">
        <div style="background-color:#ecf0f1;border-radius:10px;"class="col-md-6 col-md-offset-3">
          ...
          <legend>Add report</legend>
            ...
            <div id="server_message" class="hidden form-group">

            </div>
            <div class="form-group">
              <label for="report_name" style="width:20%;" class="col-md-2 contol-label">Name of Report</label>
                <input  style="width:60%;" class="col-md-4 form-control " type="text" id="report_name" name="report_name" value="">
            </div>
            <div class="form-group">
                <label for="report_for" style="width:20%;" class="col-md-2 ontol-label">Report for?</label>
              <input style="width:60%;"class="col-md-4 form-control" type="text" id="report_for" name="report_for" value="">
            </div>
            <div class="form-group">
              <button style="text-align:center;" class="col-md-2 col-md-offset-5 btn btn-info" type="button" id="new_report" name="new_report" value="Add">Add</button>
            </div>
            <div class="form-group">
              <button style="text-align:center;" class="col-md-2 col-md-offset-5 btn btn-default" type="button" id="new_report_done" name="new_report_done" value="Done">Done</button>
            </div>
            </fieldset>
            </form>
      ...
      <div id="add_check" style="margin-top:80px;"class=" hidden row">
        <div class="col-md-6 col-md-offset-3">
          <div class="form-group">
            ...
            <div class="form-group">
              <label for="check_count_input">Number of Checks</label>
                <input class="form-control" type="text" id="check_count_input" name="check_count_input" value="">
            </div>
            <div class="form-group">
                <label for="check_total">Total Value</label>
              <input class="form-control" type="text" id="check_total" name="check_total" value="">
            </div>
            <div class="form-group">
              <button style="width:100%;text-align:center;" class="btn btn-info" type="button" id="new_check" name="new_check" value="Add">Add</button>
            </div>
            <div class="form-group">
              <button style="width:100%;text-align:center;" class=" btn btn-default" type="button" id="new_check_done" name="new_check_done" value="Done">Done</button>
            </div>
    ...
    <script>
        ...
        jQuery(document).ready(function($){
          getReportCount = function(){
            $.ajax({
              type:"GET",
              url:"../../../ajax/ops/report_count.php",
              success:function(json){
                console.log(json['report_count']);
                var $reports = parseInt($('#report_count').text());
                if(parseInt(json['report_count']) !== $reports){
                  var options = {useEasing : true,useGrouping : true,separator : ',',decimal : '.',prefix : '',suffix : ' <span class="fa fa-anchor"></span>'};
                  //set counter variable
                  var report_counter = new countUp("report_count",$reports, parseInt(json['report_count']), 0,2.5,options);
                      report_counter.start();
                }
          ...
          getReportCount();
          setInterval(function(){ getReportCount(); }, 10000);


        });

        $('#new_check_plus').on('click',function(){
          $('#dashboard').addClass('hidden');
          $('#add_check').removeClass('hidden');

        });

        $('#new_check_done').on('click',function(){
          $('#add_check').addClass('hidden');
          $('#dashboard').removeClass('hidden');

        });

        $('#new_report_plus').on('click',function(){
          $('#dashboard').addClass('hidden');
          $('#add_report').removeClass('hidden');

        });
        $('#new_report_done').on('click',function(){
          $('#add_report').addClass('hidden');
          $('#dashboard').removeClass('hidden');
        ...
        $('#new_report').on('click',function(){
            ...
            $.ajax({
              type:'POST',
              url:'../../../ajax/ops/create_report.php?report_name='+report_name+'&report_for='+report_for,
              success:function(res){
                //$('alert_message')
                if(res==201){
                  $('#server_message').removeClass('hidden');
                  $('#server_message').append('<div style="width:60%;margin-left:20%;text-align:center;" class="btn btn-success">Report added!</div>').show('slow');
                }else if(res==500){
                    $('#server_message').removeClass('hidden');
                    $('#server_message').append('<div style="width:60%;margin-left:20%;text-align:center;" class="btn btn-warning">Fail</div>').show('slow');
                }
                console.log(res);
                clearServerMessage = function(){
                  $('#server_message').addClass('hidden').hide('slow');
                  $('#server_message').empty();
                  $('#report_name').val('');
                  $('#report_for').val('');
                }
                getReportCount();
                setTimeout(clearServerMessage,1000);
